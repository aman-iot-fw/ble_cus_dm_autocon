...
void Pairing_passcodeHandler(uint32 event, BLEAppUtil_msgHdr_t *pMsgData)
{
    uint32_t PassKey = swapedConfigData.passKey;
    BLEAppUtil_PasscodeData_t *pData = (BLEAppUtil_PasscodeData_t *)pMsgData;

    if (PassKey < 100000 || PassKey > 999999) {
        PassKey = B_APP_DEFAULT_PASSCODE;
    }

    // Send passcode response
    GAPBondMgr_PasscodeRsp(pData->connHandle, SUCCESS, PassKey);
}
...
void Pairing_pairStateHandler(uint32 event, BLEAppUtil_msgHdr_t *pMsgData)
{
    switch(event)
    {
        ...
        {
            //Start timer and turn GPIO pin High once pairing is done
            connectionFlag=1;
            LGPTimerLPF3_start(hTimer, LGPTimerLPF3_CTL_MODE_UP_PER);
            GPIO_write(CONFIG_GPIO_OUTPUT_PIN,0);
            GPIO_write(LED_PIN,0);
